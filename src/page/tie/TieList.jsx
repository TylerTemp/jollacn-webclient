...
class TieList extends Component {
  ...
  makeMediaPreview(previews, classes) {
    ...
    if (preview_length == 1) {
      ...
      return (
        <ImageList cellHeight={200} cols={1}>
          <ImageListItem key={tile.src}>
      ...
      return (
        <ImageList cellHeight={200} cols={2} spacing={1}>
          {
            previews.map(tile => (
              <ImageListItem key={tile.src}>
                { tile.type == 'video_poster' && <PlayCircleOutlineIcon classes={{ root: classNames(classes.absoluteCenter, classes.playBtn) }} />}
    ...
    return (
      <ImageList cellHeight={100} cols={rest_tile.length} spacing={1}>
        <ImageListItem key={first_tile.src} cols={rest_tile.length} rows={1}>
