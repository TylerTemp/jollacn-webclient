...
const RenderError = ({error, onRetry}) => <Alert
    severity="error"
    action={<Button color="inherit" size="small" onClick={onRetry}>
      重试
    </Button>}
  >
  {error}
...
export default({
  ...
  return <Box sx={{width: '100%', display: 'flex', justifyContent: 'center'}}>
    <Box sx={{width: '100%', maxWidth: '1100px'}}>
      <Paper sx={{margin: '0px 5px', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
        <Box sx={{maxWidth: '900px', width: '100%'}}>
          <Box sx={{padding: '10px 10px'}}>
            ...
            {medias.length > 0 && <>
              ...
              <ImageList cols={1}>
                {medias.map((item) =>
                  <RenderImageItem key={item.type==='video'? item.sources[0].src: item.src} item={item} />)}
              </ImageList>
            </>}
          </Box>
        </Box>
