...
export default({offset=0, limit=24, onResult, itemXs, itemMd, itemLg}) => {
  // const defaultLimit = 10;
  const [apiState, setApiState] = useState({loading: true, error: null, offset, limit, total: 0, tieList: []});
  ...
  const fetchTieList = offset => {
    ...
    request(reqUri)
      ...
      .then(result => {
        ...
        const stateResult = {...apiState, total: nowTotal, offset, limit: serverLimit, loading: false, error: null, tieList};
        setApiState(stateResult);
        onResult && onResult(stateResult);
      })
  ...
  return <View
    ...
    openMediaViewer={(content, medias) => setMediaViewerState({...mediaViewerState, content, medias, isOpen: true, index: 0})}
