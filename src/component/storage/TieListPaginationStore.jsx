...
import tieListCache from './TieListCache';
...
class TieListPaginationStore {
  ...
  @action fetchTiesPage(page = 1) {
    ...
    const promise = new Promise((resolve, reject) => tieListCache.fetchTieList(offset, limit, resolve, reject));
    promise
      .then(
        (api_result) => { // succeed
  ...
  @action apiResult({
    loaded = true, error = null, ties = [], limit = 50, current_page = 1, total_page = 1,
  }) {
    this.loaded = loaded;
    this.error = error;
    this.ties = ties;
